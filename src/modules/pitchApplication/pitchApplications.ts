...
export class ApplicationsController {
  ...
  approveApplication = asyncHandler(async (req: UserRequest, res: Response) => {
    ...
    const fullName = [app.first_name, app.last_name].filter(Boolean).join(" ") || "Applicant";
  
    // --- Find or create user ---
    let user = await userRepo.findOne({ where: { email: app.email }, relations: ["role"] });
    if (!user) {
