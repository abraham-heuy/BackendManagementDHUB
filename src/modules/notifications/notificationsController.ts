...
export class NotificationController {
  ...
  getNotifications = asyncHandler(async (req: UserRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
...
* - Regular user → notifications received
* - Admin → notifications received + sent
*/
getMyNotifications = asyncHandler(async (req: UserRequest, res: Response) => {
  ...
  // ✅ Only notifications SENT by this user
  const sentNotifications = await this.notifRepo.find({
    where: { sender: { id: user.id } as any },
    relations: ["sender", "user"],
    order: { created_at: "DESC" },
  });

  return res.json(sentNotifications);
});
